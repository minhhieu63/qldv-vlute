...
session_start(); 
include "conn.php";
if(isset($_SESSION)){ $kql=$_SESSION['MaKhoa'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Trang bầu bí thư lớp</title>
</head>
<body class="wrapper">
    <?php
        include "Header.php";
    ...
    <div class="wptable">
        <div class="displaytb">
		<h2 >DANH SÁCH ĐOÀN VIÊN KHOA <?php 
        
            $sql="SELECT TenKhoa from khoa where MaKhoa='$kql'";
            $result = mysqli_query($conn, $sql);$row=mysqli_fetch_assoc($result);
        
        echo mb_strtoupper($row['TenKhoa'], 'UTF-8');
		...
		<label>Lọc theo mã lớp:</label>
            ...
            $sql = "SELECT * FROM lop where MaKhoa='$kql'";
...
<script>
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    var deleteLink = $("<a href='#' class='choose'>Chọn làm BTL</a>");
    ...
    function deleteLop(maLop) {
        if (confirm("Bạn có chắc chắn muốn chọn bí thư lớp này không?")) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        alert("Chọn bí thư lớp thành công");
